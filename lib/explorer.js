...
module.exports = {
  ...
  is_unique: function(array, object, cb) {
    ...
    module.exports.syncLoop(array.length, function (loop) {
      var i = loop.iteration();
      if (object != null && array[i].addresses == object[0]) {
        unique = false;
        index = i;
        loop.break(true);
        loop.next();
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') { 
        ...
        //console.log('vout:' + i + ':' + txid);
          ...
          module.exports.is_unique(arr_vout, vout[i].scriptPubKey.addresses, function(unique, index) {
            if (unique == true) {
              // unique vout
              module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
                arr_vout.push({addresses: vout[i].scriptPubKey.addresses, amount: amount_sat});
                loop.next();
              });
            } else {
              // already exists
              module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
                if(arr_vout[index] != null){
